...
namespace PlayerWithAnimation
{
    ...
    public class PlayerMover_PlayerWithAnimation : MonoBehaviour
    {
        ...
        private Collider CurrentGround()
        {
            ...
            if (Physics.Raycast(origin, Vector3.down, out RaycastHit hit, 10.0f, groundLayer))
            {
        ...
        private void MoveToTarget(Vector3 targetPos, float dt)
        {
            Vector3 currentPos = rb.position + Vector3.up * footOffset;
            Vector3 moveDelta = targetPos - currentPos;
            
            Vector3 moveDir = moveDelta.normalized;
            float mag = Mathf.Min(moveDelta.magnitude , 10.0f * dt);
            if (Physics.Raycast(currentPos, moveDir, out RaycastHit hit, mag+ 1.0f, obstacleLayer))
            {
                Debug.Log("Obstacle detected");
                float safeDist = Mathf.Max(0, hit.distance- 0.5f);
                moveDelta = moveDir * safeDist;
            }
            else
            {
                Debug.Log("Just plane");
                moveDelta = moveDir * mag;
            }
    ...
    public class PlayerMover_PlayerWithAnimation : MonoBehaviour
    {
        ...
        private void HandleMove(float dt)
        {
                ...
                groundLayer))
            {
                ...
                {
                    Debug.Log("same");
                    targetPos = transform.position + (moveDir * 0.2f * dt);
                    MoveToTarget(targetPos ,dt );
        ...
        private void MoveToTarget(Vector3 targetPos , float dt)
        {
            ...
            originMag = Mathf.Max(originMag, 2.0f * dt);
            moveDelta = moveDir * originMag;
            if(Physics.Raycast(currentPos, moveDir, out RaycastHit hit, originMag +0.4f, ~groundLayer))
            {
                Debug.Log("222");
